...
./compilar.sh
\end{lstlisting}
\archivo{compilar.sh} genera tablas y figuras desde \archivo{datos/} y compila \archivo{main.tex}. No llama a \texttt{mpirun}, no cambia configuraciones de los workers ni repite los experimentos.
...
\end{lstlisting}
\texttt{T\_Dist} reúne \texttt{Scatterv} y \texttt{Bcast}; esta versión no registra cada una por separado. El valor de \texttt{T\_Gather} puede incluir espera mientras otros procesos aún distribuyen o calculan. Por eso no se suma a los otros máximos para reconstruir el total.

\clearpage
\section{Cómo registrar los MB en una próxima ejecución}

La batería actual no guarda MB. Para una futura prueba puntual se incluye \archivo{medir_trafico.sh}. Lee los contadores de la interfaz elegida en los cuatro nodos y llama al lanzador existente una sola vez. Consulta los workers por Ethernet para reducir el tráfico de control agregado a una medición Wi-Fi; al medir Ethernet esas consultas sí pueden contribuir al contador. Sigue siendo una medida de interfaces, sin aislamiento de tráfico ajeno.
...
El paquete del informe incluye una copia de la evidencia necesaria para reproducir sus tablas y gráficos. \archivo{datos/procedencia.json} relaciona cada copia con su ruta original y su hash SHA-256; \archivo{datos/validacion.json} resume las comprobaciones. \archivo{datos/metricas_informe.csv} conserva las métricas calculadas sin el redondeo de las tablas.

\begin{center}\small
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{5.5cm}X}
\toprule
Ruta dentro del informe & Contenido \\
\midrule
\archivo{datos/ethernet/} & 25 resultados actuales y sus logs por cable. \\
\archivo{datos/wifi/} & 25 resultados actuales y sus logs Wi-Fi. \\
...
\archivo{src/} & Fuentes actuales de trapecio, matrices y kernels. \\
\archivo{scripts/} & Copias de lanzadores, medición, gráficos y configuración SSH. \\
\archivo{generar_datos.py} & Lee la evidencia, valida consistencia y genera tablas y cinco figuras. \\
\archivo{medir_trafico.sh} & Herramienta opcional para una futura medición de MB. \\
\bottomrule
\end{tabularx}\end{center}

El contexto actualizado del proyecto orientó la revisión, pero las cifras finales se tomaron de los CSV y logs. Se corrigieron diferencias de tiempo y atribuciones entre series: 1.053217 s para trapecio grande Wi-Fi actual, 906.351 s en la serie original con 8 procesos, y 1112 MB en la serie posterior con 16 procesos. Los MB actuales de 96 procesos quedan sin dato observado.
