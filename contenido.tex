...
\begin{idea}
\textbf{Lo que encontramos.} Con $10^{11}$ trapecios, Ethernet pasa de 66.114 s con un proceso a 0.964 s con 96: una aceleración de 68.56 veces. En matrices de $3072\times3072$, el mejor punto de la batería actual es \textbf{24 procesos en una computadora}: 0.512 s por Ethernet y 0.553 s en la configuración Wi-Fi. Con 96 procesos, los tiempos aumentan a 6.175 s y 101.042 s, respectivamente.
\end{idea}

\subsection{Qué datos usa este informe}
La batería actual contiene \textbf{25 configuraciones por red}, con una ejecución de cada configuración: diez de trapecio y quince de matrices. Los resultados originales se conservaron en:
\begin{itemize}
...
\end{itemize}
Las rutas parten de \archivo{/home/alumno16/}. Las tablas se generaron leyendo los CSV y comprobando su coincidencia con las líneas \archivo{RESULT_TRAP} y \archivo{RESULT_2D} de los 50 logs. No se ejecutaron nuevos benchmarks para redactar este documento.

El código, los datos y la fuente de este informe están versionados en el repositorio privado \url{https://github.com/dtamotu/cluster-mpi-96}.

Los registros de madrugada y mediodía se usan como \textbf{antecedentes identificados por hora, versión, número de procesos y repeticiones}. No se mezclan con la batería actual para calcular promedios. En particular, la prueba de 906 s y la prueba que transmitió 1112 MB fueron ejecuciones diferentes.
...
Se calcula $C=A B$, con matrices cuadradas de dobles. La batería actual ejecuta:
...
Se ejecutan las mismas 25 configuraciones en las dos redes. En las corridas actuales se completó primero Ethernet y después Wi-Fi, sin alternancia ni aleatorización. Se realizó una sola ejecución por configuración, por decisión de simplificar la batería.

\begin{idea}
Los resultados actuales describen \textbf{lo observado en esas ejecuciones}. Una medición permite calcular tiempo, speedup y eficiencia, pero no permite estimar dispersión, intervalos de confianza o estabilidad. Las diferencias pequeñas entre redes deben interpretarse con especial cuidado.
\end{idea}
...
El tiempo \archivo{T_Total} \textbf{no incluye} iniciar \texttt{mpirun}, arrancar por SSH, \archivo{MPI_Init}, preparar las matrices ni calcular los checksums finales. No se utiliza tiempo pared externo en las comparaciones actuales.
...
Se cotejaron las 50 líneas de resultado con sus CSV. Las 30 ejecuciones actuales de matrices presentan las mismas huellas para cada tamaño, a la precisión impresa. Son verificaciones de consistencia, no una demostración de igualdad elemento a elemento. Los scripts de medición extraen el tiempo; la revisión de huellas se hizo al generar este informe.
...
La siguiente tabla usa las \textbf{medianas de tres ejecuciones locales} de \archivo{resultados_bloques.json}, para $N=3072$. Es un antecedente del cambio de kernel; no pertenece a las 50 mediciones actuales.
...
\clearpage
\section{Resultados actuales del trapecio}
...
\clearpage
\section{Resultados actuales de multiplicación de matrices}
...
En los tres tamaños, \textbf{24 procesos es el mejor punto de la batería actual}. Para $N=3072$, Ethernet baja de 3.816321 s a 0.512022 s: mejora 7.45 veces. Al pasar a 48 procesos el tiempo aumenta a 2.815416 s, y con 96 llega a 6.174707 s, incluso por encima de la referencia de un proceso.
...
Para cada tamaño, las diez ejecuciones actuales (cinco valores de $P$ por dos redes) imprimen la misma suma de la diagonal y la misma suma ponderada:
...
\clearpage
\section{Qué ocurrió en las pruebas de madrugada}

\subsection{Primera serie: hasta 15 minutos con el programa original}
Entre las 00:37 y 01:37 del 24/09 se repitieron pruebas del ejecutable \archivo{/tmp/mpi_matrix_profiling}, con bucle \texttt{ikj}. El archivo \archivo{resultados_profiling_repeticion.json} separa el tiempo de \archivo{MPI_Bcast} del de \archivo{MPI_Scatterv}. Para $N=3072$:
\input{tablas/madrugada_original}

La ejecución de \textbf{906.351 s} empezó a las 00:58:05 y utilizó \textbf{8 procesos, dos en cada nodo}. El mayor tiempo de \archivo{Bcast} fue 889.744 s y el de cálculo, 1.735 s. La segunda repetición de la misma configuración tardó 322.513 s y tuvo un tiempo de cálculo casi idéntico. Esto localiza la variación en la comunicación y las esperas, no en un aumento del trabajo matemático.
...
\textbf{Los 357--358 MB son mediciones de esta serie histórica, no de la batería actual de 96 procesos.} La batería actual conserva tiempos y logs, pero no muestrea los contadores de red. Tampoco se deben presentar los 5.29--5.64 s de 1D-tiled con 16 procesos como repeticiones Wi-Fi: esos valores son de Ethernet.
...
El modelo de 339.74 MB es compatible en orden de magnitud con los 357--358 MB históricos de ciertas configuraciones. Para la batería actual con 96 procesos, el tráfico medido es \textbf{no disponible}. Por eso no presentamos 357 MB como medición actual ni calculamos un ancho de banda observado de la corrida actual usando esa cifra supuesta.

\clearpage
\section{Conclusiones y límites de lo que podemos defender}

\begin{enumerate}
\item \textbf{El tipo de problema determina si conviene distribuirlo.} Trapecio grande obtiene 68.56 veces la velocidad de un proceso por Ethernet con 96 procesos. Matrices de $N=3072$ funciona mejor con 24 procesos en un nodo que con 96 en cuatro nodos.
\item \textbf{La red explica gran parte de la penalización de matrices.} Con 96 procesos, el cálculo máximo ronda 0.10 s en ambas redes, mientras el total es 6.175 s por cable y 101.042 s por Wi-Fi. Las fases de distribución y reunión concentran los tiempos grandes.
...
\item \textbf{Las mediciones actuales permiten comparar estas corridas, no estimar estabilidad.} Se hizo una ejecución por caso, en horarios diferentes para cada red. La variación histórica muestra por qué no debe describirse un tiempo Wi-Fi como garantizado.
\end{enumerate}

\subsection{Qué cambió respecto del informe anterior}
Ahora sí se usa Ethernet, ya no queda como una propuesta futura. Se incorporó trapecio, se llegó a 96 procesos y se incluyó un punto explícito con un nodo completo. Los lanzadores son Bash, los tiempos se guardan en CSV y los análisis calculan speedup y eficiencia al terminar. Se conserva la ejecución única elegida para simplificar la batería y se excluye el tiempo pared externo.
